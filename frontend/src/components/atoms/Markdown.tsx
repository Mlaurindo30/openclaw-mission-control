  ...
  code: (rawProps) => {
    ...
    if (isInline) {
      return (
        <code
          className={cn(
            "rounded bg-[var(--surface-muted)] px-1 py-0.5 font-mono text-[0.85em] text-[var(--text)]",
            className,
          )}
          {...props}
        >
...
const MARKDOWN_TABLE_COMPONENTS: Components = {
  ...
  th: ({ node: _node, className, children, ...props }) => (
    <th
      className={cn(
        "border border-[var(--border)] px-3 py-2 text-left text-xs font-semibold",
        className,
      )}
      {...props}
    >
  ...
  td: ({ node: _node, className, children, ...props }) => (
    <td
      className={cn("border border-[var(--border)] px-3 py-2 align-top", className)}
      {...props}
    >
      {renderMentions(children)}
    </td>
