...
export function linkifyCell({
  ...
  if (block) {
    return (
      <Link href={href} title={title} className={cn("group block", className)}>
        <p
          className={cn(
            "text-sm font-medium text-[var(--text)] group-hover:text-blue-600",
            labelClassName,
          )}
        >
          {label}
        ...
        {subtitle != null ? (
          <p className={cn("text-xs text-[var(--text-quiet)]", subtitleClassName)}>
            {subtitle}
          </p>
        ) : null}
      </Link>
  ...
  return (
    <Link
      ...
      title={title}
      className={cn(
        "text-sm font-medium text-[var(--text-muted)] hover:text-blue-600",
        className,
      )}
    >
      {label}
