...
export function DashboardShell({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-app text-strong" data-sidebar={sidebarOpen ? "open" : "closed"}>
      <header className="sticky top-0 z-50 border-b border-[var(--border)] bg-[var(--surface)] shadow-sm dark:border-[var(--border)] dark:bg-surface">
        <div className="flex items-center py-3">
          <div className="flex items-center px-4 md:px-6 md:w-[260px]">
            {isSignedIn ? (
              <button
                type="button"
                className="mr-3 rounded-lg p-2 text-[var(--text-muted)] hover:bg-[var(--surface-muted)] text-[var(--text-muted)] hover:bg-[var(--surface-muted)] md:hidden"
          ...
          <SignedIn>
            <div className="ml-auto flex items-center gap-3 px-4 md:px-6">
              ...
              <div className="hidden text-right lg:block">
                <p className="text-sm font-semibold text-[var(--text)] text-[var(--text)]">
                  {displayName}
                </p>
                <p className="text-xs text-[var(--text-muted)] text-[var(--text-muted)]">Operator</p>
              </div>
